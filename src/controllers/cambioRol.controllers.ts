...
import { SolicitudCambioRolService } from '../services/cambioRol.service';

export class SolicitudCambioRolController {

  // Para que el usuario cree una solicitud de cambio de rol
 public static async crearSolicitud(
  ...
  try {
    ...
    if (!usuarioId) {
      ...
      const resultado = await SolicitudCambioRolService.crearSolicitudCambioRol(
        usuarioId,
        { rolSolicitado, motivoSolicitud, datosAdicionales }
      );
  ...
  public static async obtenerMisSolicitudes( req: Request & { user?: { id: string; rol: string } }, res: Response): Promise<void> {
    try {
      ...
      const resultado = await SolicitudCambioRolService.obtenerSolicitudesUsuario(usuarioId);
  ...
  public static async obtenerSolicitudesPendientes( req: Request & { user?: { id: string; rol: string } }, res: Response): Promise<void> {
    try {
      ...
      const resultado = await SolicitudCambioRolService.obtenerSolicitudesPendientesDetalladas();
  ...
  public static async responderSolicitud( req: Request & { user?: { id: string; rol: string } }, res: Response): Promise<void> {
    try {
      // Verificar que sea administrador
      if (req.user?.rol !== 'administrador') {
        res.status(403).json({
      ...
      const resultado = await SolicitudCambioRolService.responderSolicitud(
        solicitudId,
        adminId,
        { decision, comentario }
      );
