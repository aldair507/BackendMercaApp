...
import { VentaData, VentaResponse } from "../interfaces/venta.interface";
  ...
  static async registrarVenta(
    ...
    try {
      ...
      await PersonaModel.updateOne(
        { idPersona: vendedorId },
        { $push: { ventasRealizadas: nuevaVenta.idVenta.trim() } }
      );
      ...
      if (!esEfectivo) {
        ...
        if (!compradorInfo) {
          throw new Error(
            "Los datos del comprador son requeridos para pagos con MercadoPago"
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Email, nombre y apellido del comprador son requeridos para MercadoPago"
          );
        }
        ...
        if (!mercadoPagoResponse.success) {
          ...
          throw new Error(
            `Error al crear preferencia de MercadoPago: ${mercadoPagoResponse.error}`
          );
        }
      }
...
static async obtenerVenta(ventaId: string) {
  try {
    ...
    const venta = await VentaModel.findOne({ idVenta: cleanVentaId });
   
    
    if (!venta) {
      // Verificar que exista alguna venta con un ID similar
      const ventasExistentes = await VentaModel.find({}, { idVenta: 1 }).limit(5);
  ...
  static async obtenerVentasPorVendedor(idPersona: string) {
    try {
      const [persona, ventas] = await Promise.all([
        PersonaModel.findOne({ idPersona })
          .select("idPersona nombrePersona apellido")
          .lean(),
