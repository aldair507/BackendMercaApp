import { SolicitudCambioRolModel } from "../models/cambioRol.model";
...
export class SolicitudCambioRolService {
  ...
  public static async crearSolicitudCambioRol(
    usuarioId: string,
    datosSolicitud: {
    ...
    success: boolean;
    data?: ISolicitudCambioRol;
    error?: string;
  }> {
    try {
      // Verificar que el usuario existe
      const usuarioExistente = await PersonaService.obtenerUsuarioPorId(
        usuarioId
      );
      ...
      const solicitudPendiente = await SolicitudCambioRolModel.findOne({
      ...
      // Crear la solicitud
      const nuevaSolicitud = new SolicitudCambioRolModel({
        idSolicitud,
        usuarioSolicitante: usuarioId,
  ...
  public static async obtenerSolicitudesPendientes(): Promise<{
    success: boolean;
    data?: ISolicitudCambioRol[];
    error?: string;
  }> {
    try {
      const solicitudes = await SolicitudCambioRolModel.find({
        estadoSolicitud: "pendiente",
      })
  ...
  public static async obtenerSolicitudesPendientesDetalladas(): Promise<{
    success: boolean;
    data?: any[];
    error?: string;
    total?: number;
    ...
    try {
      ...
      const usuarios = await Promise.all(
        usuariosIds.map(async (usuarioId) => {
          const usuario = await PersonaService.obtenerUsuarioPorId(
            usuarioId.toString()
          );
          return {
            id: usuarioId,
      ...
      const solicitudesFormateadas = solicitudes.map((solicitud) => {
        const usuarioData = usuariosMap.get(
          solicitud.usuarioSolicitante.toString()
        );
      ...
      console.error(
        "Error obteniendo solicitudes pendientes detalladas:",
        error
      );
      return {
        success: false,
  ...
  public static async obtenerSolicitudesPendientesDetalladasOptimizada(): Promise<{
    success: boolean;
    data?: any[];
    error?: string;
    total?: number;
    ...
    try {
      ...
      const usuarios = await PersonaModel.find({
        idPersona: { $in: usuariosIds },
      })
        .select(
          "idPersona nombrePersona apellido correo identificacion rol telefono ciudad"
        )
        .lean();
  ...
  public static async responderSolicitud(
    solicitudId: string,
    adminId: string,
    respuesta: {
    ...
    try {
      const solicitud = await SolicitudCambioRolModel.findOne({
      ...
      if (respuesta.decision === "aprobada") {
        ...
        await PersonaService.actualizarDatosUsuario(
          usuarioId.toString(),
          datosActualizacion,
          true // Permitir cambio de rol
        );
      }
 ...
 public static async obtenerSolicitudesUsuario(usuarioId: string): Promise<{
  success: boolean;
  data?: any[]; // Cambié a any[] porque agregamos datos de usuario
  error?: string;
}> {
  try {
    ...
    const usuarioData = await PersonaModel.findOne({
      idPersona: usuarioId,
    })
  ...
  public static async cancelarSolicitud(
    solicitudId: string,
    usuarioId: string
  ): Promise<{
    success: boolean;
    ...
    try {
      ...
      await SolicitudCambioRolModel.deleteOne({ _id: solicitud._id });
  ...
  public static async obtenerEstadisticasSolicitudes(): Promise<{
    success: boolean;
    data?: any;
    error?: string;
  }> {
    try {
      const estadisticas = await SolicitudCambioRolModel.aggregate([
        {
          $group: {
            _id: "$estadoSolicitud",
            total: { $sum: 1 },
      ...
      const estadisticasPorRol = await SolicitudCambioRolModel.aggregate([
        {
          $group: {
            _id: "$rolSolicitado",
            total: { $sum: 1 },
