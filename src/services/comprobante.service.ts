...
export class ComprobanteService {
  static async generarComprobante(
    ...
    try {
      ...
      const venta = await VentaModel.findById(ventaId).lean();
  ...
  private static generarFacturaEstilizada(doc: PDFDocumentType, venta: any) {
    ...
    // Subtotal
    const subtotal = venta.productos.reduce((acc: number, producto: any) => {
    ...
    // Impuestos
    const totalImpuestos = venta.productos.reduce((acc: number, producto: any) => {
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        "Gracias por confiar en MercaApp - Su socio comercial en Popayán",
        50,
        pieY + 20,
        { align: "center", width: doc.page.width - 100 }
      )
      .text(
        "Este documento es una representación impresa de una factura electrónica generada por MercaApp",
        50,
        pieY + 35,
        { align: "center", width: doc.page.width - 100 }
      )
  ...
  private static generarTicketEstilizado(doc: PDFDocumentType, venta: any) {
    ...
    venta.productos.forEach((producto: any, index: number) => {
      ...
      doc
        ...
        .font("Helvetica")
        .text(`   $${producto.precioUnitario.toLocaleString('es-CO')} c/u → $${producto.subtotal.toLocaleString('es-CO')}`, 
          { indent: 20 });
      ...
      if (producto.impuestos > 0) {
        doc
          .fontSize(9)
          .fillColor('#666666')
          .text(`   IVA (${producto.impuestos}%): $${impuesto.toLocaleString('es-CO')}`, 
            { indent: 20 });
      }
  ...
  static async obtenerHistorialComprobantes(ventaId: string) {
    try {
      ...
      const files = fs
        ...
        .map((file) => ({
          nombre: file,
          rutaCompleta: path.join(comprobantesDir, file),
          fecha: fs.statSync(path.join(comprobantesDir, file)).mtime,
        }));
