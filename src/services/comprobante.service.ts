...
type PDFDocumentType = typeof PDFDocument extends new (
  ...args: any[]
) => infer T
  ? T
  : never;
...
export class ComprobanteService {
  static async generarComprobante(
    ventaId: string,
    tipo: "ticket" | "factura" = "ticket",
    ...
    try {
      ...
      const venta = await VentaModel.findOne({ idVenta: ventaId.trim() }).lean();
      
      if (!venta) {
        console.error("❌ Venta no encontrada con ID:", ventaId);
        // Debug: mostrar algunas ventas existentes
        const ventasExistentes = await VentaModel.find({}, { idVenta: 1 }).limit(3).lean();
      ...
      const productosIds = [
        ...new Set(venta.productos.map((p: any) => p.idProducto)),
      ];
      ...
      const productosMap = new Map(
        productos.map((p: any) => [p.idProducto, p])
      );
      ...
      venta.productos = venta.productos.map((productoVenta: any) => {
      ...
      const doc: PDFDocumentType = new PDFDocument({
      ...
      const filePath = path.join("C:/MercaApp_Comprobantes/facturas", fileName);

      // Crear el directorio si no existe
      if (!fs.existsSync(path.dirname(filePath))) {
        fs.mkdirSync(path.dirname(filePath), { recursive: true });
        console.log("✅ Directorio creado:", path.dirname(filePath));
      }
  ...
  private static generarFacturaEstilizada(doc: PDFDocumentType, venta: any) {
    ...
    if (venta.productos && Array.isArray(venta.productos)) {
      venta.productos.forEach((producto: any, index: number) => {
    ...
    const subtotalSinDescuento = venta.productos?.reduce((acc: number, p: any) => 
      acc + ((p.cantidadVendida || 0) * (p.precioUnitario || 0)), 0) || 0;
    const descuentoTotal = venta.productos?.reduce((acc: number, p: any) => 
      acc + ((p.descuento || 0) * (p.cantidadVendida || 0) * (p.precioUnitario || 0) / 100), 0) || 0;
    
    // ✅ CORRECCIÓN PRINCIPAL: Calcular total real desde los productos si totalVenta es 0
    const totalCalculado = venta.productos?.reduce((acc: number, p: any) => 
      acc + (p.subtotal || 0), 0) || 0;
    ...
    doc.text(
      'Gracias por confiar en MercaApp. Este documento es un comprobante oficial de venta.',
      50, yPos,
    ...
    doc.text(
      `Documento generado electrónicamente el ${new Date().toLocaleString('es-CO')}`,
      50, yPos,
      { align: 'center', width: 500 }
    );
  }
  ...
  private static generarTicketEstilizado(doc: PDFDocumentType, venta: any) {
    const fecha = new Date(venta.fechaVenta).toLocaleDateString('es-CO');
    const hora = new Date(venta.fechaVenta).toLocaleTimeString('es-CO', {
      hour: '2-digit',
      minute: '2-digit'
